...
# MACHINE examples: vuduo vusolo vuultimo vuuno
...
BBLAYERS ?= \
	...
	$(CURDIR)/meta-blackpole \


CONFFILES = \
	$(TOPDIR)/env.source \
	$(TOPDIR)/conf/blackpole.conf \
...
CONFDEPS = \
	...
	$(DEPDIR)/.blackpole.conf.$(BLACKPOLE_CONF_HASH) \
...
all: init
	@echo
	@echo "Openembedded for the Black Pole OE 2 environment has been initialized"
	@echo "properly. Now you can start building your image, by doing either:"
	@echo
	@echo " MACHINE=$(MACHINE) make image"
	@echo "	or"
	@echo " cd $(BUILD_DIR) ; source env.source ; bitbake blackpole-enigma2-image"
	@echo
...
image: init
	...
	@. $(TOPDIR)/env.source && cd $(TOPDIR) && bitbake blackpole-enigma2-image
...
update:
	...
	if [ "$$HASH" != "`$(XSUM) $(MAKEFILE_LIST)`" ]; then \
		...
		echo "The Black Pole OE is now up-to-date."; \
	fi
...
BLACKPOLE_CONF_HASH := $(call hash, \
	'BLACKPOLE_CONF_VERSION = "1"' \
	'CURDIR = "$(CURDIR)"' \
...
$(TOPDIR)/conf/blackpole.conf: $(DEPDIR)/.blackpole.conf.$(BLACKPOLE_CONF_HASH)
	@echo 'Generating $@'
	@test -d $(@D) || mkdir -p $(@D)
...
$(TOPDIR)/conf/local.conf: $(DEPDIR)/.local.conf.$(MACHINE).$(LOCAL_CONF_HASH)
	...
	@echo 'require $(TOPDIR)/conf/blackpole.conf' >> $@
